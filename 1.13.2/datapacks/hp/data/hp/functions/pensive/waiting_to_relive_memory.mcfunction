...
execute as @s[tag=!cancelPensive] store success score @s tmp run title @s actionbar ["",{"text":"모든 사람이 펜시브와 상호작용할 때까지 기다리세요 (","color":"gold"},{"score":{"name":"@s","objective":"tmp2"},"color":"gold"},{"text":" / ","color":"gold"},{"score":{"name":"@s","objective":"tmp3"},"color":"gold"},{"text":") ","color":"gold"}]
