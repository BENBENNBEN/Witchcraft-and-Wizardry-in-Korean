...
# Ear muffs
execute if entity @s[tag=earmuffs] as @p[tag=activePlayer,scores={pickUpTimer=0},tag=!cutsceneOverrideActionbar,tag=!flyingBroom] store success score @s tmp run title @s actionbar ["",{"text":"귀마개를 주으려면  ","color":"gold"},{"text":"⌡","color":"white"},{"text":" 를 누르세요.","color":"gold"}]
