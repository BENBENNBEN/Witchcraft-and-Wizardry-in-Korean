...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = coralynnAvellana p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = coralynnAvellana p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = coralynnAvellana p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = coralynnAvellana p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 안녕 얘야. 물건 사려고?\\n \"}"}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=2}] run data merge entity @s {CustomName:"{\"text\":\"≤ 다이애건 앨리에 대해 얘기해주세요\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 혹시 나가서 직접 재료를 구하기 귀찮으면, 다이애건 앨리에는 마법약 재료를 파는 곳이 많아. 여기 없으면 내 친구 윌튼 햄록이 하는 가게에 가보던가. 조금만 가면 있는 슬러그 앤 지글러 마법약 상점이야. \\n \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 아니면 멀페퍼 씨의 상점에 가보던가. 윌튼이랑 사이 안 좋긴 하지만. \\n\\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 마법약 양조에 필요한 식물이나 여러 재료를 팔아. 그런데 요즘 가게 운영하는 게 아주 힘들어졌어. 멀페퍼 씨가 이 거리에 가게를 낸 이후로 수익이 엄청나게 줄었거든. 나뿐만이 아니야. 이 거리에 모든 가게를 몰아내고 있다니까. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
