...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = jasperArbutus p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = jasperArbutus p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = jasperArbutus p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = jasperArbutus p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 어서 오세요. 호그스미드 지점 중고 서점입니다. 어떻게 도와드릴까요? \"}"}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 특이한 냄새 나는 책을 좋아하니? 음... 우리 서점의 책들은 대부분 독특한 냄새를 풍기거든. 우리 서점에서는 중고 책을 주로 팔고 있단다. 그게 싫다면 톰스 앤 스크롤이나 옵스큐러스 서점이나 다이애건 앨리에 있는 플러러시 앤 블러츠에 가봐. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 호그스미드는 조그마한 마을이야. 하지만 항상 호그와트 학생들로 북적이지. 다이애건 앨리에도 브로더릭 브라이스가 운영하는 중고 서점 지점이 있어. 안 가봤으면 한 번 가봐. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 고마워! \\n\\n\\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
