...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = fatFriar p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = fatFriar p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = fatFriar p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = fatFriar p4convOverride
...
################################
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"≤ 후플푸프 학생이었나요? \"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0}] store success score @s tmp run scoreboard players set @s convState 1
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=2}] run data merge entity @s {CustomName:"{\"text\":\"≤ 어떻게 죽었나요? \"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=3}] run data merge entity @s {CustomName:"{\"text\":\"≤ 왜 아직도 호그와트에 있나요? \"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 대뜸 그런 우울한 얘기를 물어보는군. 뭐, 내 대답까지 우울할 필요는 없지. 그냥 어이없는 죽음이었어. 호그와트에서 졸업하고 나서 성직자가 되었는데 평생을 기부 좀 해달라고 구걸만 하다 죽었다니까. \\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 유령이 되고 나서 호그와트로 돌아왔지. 내 인생에서 가장 즐거운 추억들이 있는 곳이고, 죽고 나서도 가장 즐거운 곳이라서 아주 당연하게 후플푸프의 기숙사 유령이 됐어. 지금은 다른 유령들과 함께 만족스러운 사후 생활을 하고 있지. \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
