...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = danikaAsp p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = danikaAsp p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = danikaAsp p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = danikaAsp p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 반가워. 여기는 위저딩 무선 네트워크 본부, 짧게 줄여서 WWN이야. \"}"}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 마법 세계로 온 지 얼마 안 됐니? WWN은 마법사들에게 인기 있는 무선국이야. 주로 머글들이 라디오 방송국이라고 부르는 곳 말이야. 우리 무선국은 마법으로 작동해, 그 전기 같은 헛소리가 아니라. \\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 아! 드디어. 기다리고 있었어. \\n\\n\\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
