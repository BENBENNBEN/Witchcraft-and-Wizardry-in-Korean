...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = hogsmeade60 p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = hogsmeade60 p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = hogsmeade60 p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 버티 보트의 온갖 맛이 나는 강낭콩 젤리 먹지 마. 내 동생은 늦었어. 이미 중독되어버렸거든. \\n\\n\\n\\n\"}"}
