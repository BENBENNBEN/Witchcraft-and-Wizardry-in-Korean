...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = catherynLegalos p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = catherynLegalos p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = catherynLegalos p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = catherynLegalos p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 여기는 문구점이야. 호그스미드에서는 유일한 문구점이지. 깃펜이라던가 양피지, 호그와트 학생들에게 필요한 문구 용품을 팔고 있어. 종코의 장난감 가게나 허니듀크만큼 인기는 없지만, 숙제하려면 우리 가게에서 잉크나 양피지를 사야 한다고. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 조용한 마을이야. 나랑 우리 남편 둘 다 오러였다가 은퇴하고 나서 이 마을에서 조용히 살고 있어. 호그스미드가 작아서 그런가 사건이라고는 없더라. 아무 일 없이 평화롭게 살기 딱 맞지. \\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
