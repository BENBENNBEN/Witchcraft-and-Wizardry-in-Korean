...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = madamPuddifoot p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = madamPuddifoot p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = madamPuddifoot p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = madamPuddifoot p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 푸디풋 부인의 찻집에 잘 왔단다, 얘야. 뭘 도와줄까? \"}"}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=2}] run data merge entity @s {CustomName:"{\"text\":\"≤ 가게에 대해 얘기해주세요 \"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 오, 그냥 자그마한 카페야. 호그와트 커플들이 와서 차나 커피를 마시곤 하지. 특히 발렌타인 데이에. \\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
