...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 문카프는 수줍음이 많은 마법 생물이라 보름달이 뜰 때만 나와. 짝짓기 때는 뒷발로 일어선 채로 복잡하고 정교한 구애의 춤을 추는데, 그러면 밀밭에 기하학적인 무늬가 생겨. 머글들에게 풀리지 않는 미스터리를 남기는 거지. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 머트랩은 길들이지 않으면 공격적이야. 대부분 물린다고 해도 부상이 심각하지 않지만, 가끔 심한 반응을 보이는 사람들이 있어. 항문에서 불꽃을 쏘는 증상을 보이는데 다행히 24시간 이상 지속하지 않아. 머트랩 등에서 자라는 촉수를 사용해서 마법약으로 만들 수 있어. \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 보우트러클은 곤충을 먹고 나무에 사는 작은 마법 생물이야. 나무껍질과 나뭇가지가 뭉친 것처럼 생겨서 나무와 구별하기 힘들다고 해. \\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
