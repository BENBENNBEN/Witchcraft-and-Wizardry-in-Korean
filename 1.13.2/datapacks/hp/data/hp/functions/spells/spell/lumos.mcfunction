...
execute as @s[scores={currentSpell=1,selectedSlot=1},tag=!inLumosArea,tag=!preventDoubleExecution,tag=!inLumos,tag=hasDoubleRightClicked] run title @s subtitle {"text":"이미 충분히 밝습니다!","bold":false,"color":"red"}