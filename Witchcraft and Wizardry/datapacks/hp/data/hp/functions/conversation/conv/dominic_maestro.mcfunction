...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = dominicMaestro p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = dominicMaestro p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = dominicMaestro p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = dominicMaestro p4convOverride
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"≤ 어디서 음악을 배우셨어요?\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 나는 영국에서 전 세계 음악을 공부했지. 사람들은 시간을 들여서 연습해야만 좋은 음악가가 된다고 하지만, 나는 그게 없어도 되는 천재라고 생각하네. 호그와트 다닐 적에도 상을 받았었으니까. 그 개구리 합창단 출신이야, 내가. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
