...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = zacheriasPodmore p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = zacheriasPodmore p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = zacheriasPodmore p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = zacheriasPodmore p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 글러버스 먼디 여행사에 오신 걸 환영합니다! 어떻게 도와드릴까요? \\n \"}"}
...
################################
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"≤ 무슨 투어가 있나요? \"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 원하시는 것 전부 있습니다. 코르크, 앤트워프, 헬싱키, 고텐부르크, 에든버러 어디든 빗자루로 여행하고 싶으신 곳을 말씀하세요. 별로인가요? 마법 세계 역사의 거물들이 잠들어 있는 무덤 투어는 어떠십니까? 서쪽 여행을 원하시면, 미합중국 마법 의회의 지팡이 승인 절차를 도와드립니다. 영국 머글들의 삶이 궁금하시다면 순간이동 투어도 준비되어 있죠. \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
