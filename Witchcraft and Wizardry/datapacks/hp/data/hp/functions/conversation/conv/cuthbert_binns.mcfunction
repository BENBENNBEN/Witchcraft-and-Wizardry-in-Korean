...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = cuthbertBinns p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = cuthbertBinns p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = cuthbertBinns p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = cuthbertBinns p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"[{\"text\":\" 좋은 아침이군. 커스버트 빈스 교수라고 한다. \\n\\n\\n\\n\\n\"}]"}
...
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=1}] run scoreboard players set cuthbertBinns p1convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=2}] run scoreboard players set cuthbertBinns p2convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=3}] run scoreboard players set cuthbertBinns p3convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=4}] run scoreboard players set cuthbertBinns p4convOverride 1
...
################################
#===============================
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 마법의 역사에 대해 질문 있나? \\n\"}"}
# execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 그래, 마법의 역사는 이곳 호그와트 마법학교에서 가르치는 핵심 수업 과목이야. 말 그대로 마법의 역사에 관해 탐구하고 배우는 과목이지. 역사 사실을 말이다. 고블린 반란이나 거인 전쟁 같은. 신화나 전설이 아니라.  또한, 수업 특성상, 마법 실습이 필요 없어. 그러니 지팡이도 필요 없지. 받아적을 양피지와 깃펜만 있으면 돼. \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 아, 정말 오래되었지. 평생을 가르쳤고, 죽은 이후로도 줄곧 가르치고 있으니까. 마침 내가 교무실에서 잠을 자다가 죽었거든. 알맞은 자리에서 죽은 셈이지. 그리고 유령이 된 채로 일어나 계속 가르쳐왔어. \\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 내가? 세상에. \\n\\n\\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 지하 도서관에 관한 기록은 1612년 고블린 반란 관련 문서에서 본 기억이 있군. 그 문서는 아마 도서관 제한 구역에 있을 거다. \\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0}] run tag @s add hasAskedProfessorBinnsAboutTheUndergroundLibrary
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0}] store success score @s tmp run scoreboard players set @s convState -1
#-------------------------------
