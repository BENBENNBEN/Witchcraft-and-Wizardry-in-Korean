...
execute as @s[tag=use] as @e[tag=convOption] if score @s con = @p[tag=activePlayer] questConvOffset run data merge entity @s {CustomName:"{\"text\":\"±ˈˈ 모든 물품을 가져왔어\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
execute as @s[tag=use] as @e[tag=convOption] if score @s con = @p[tag=activePlayer] questConvOffset run data merge entity @s {CustomName:"{\"text\":\"±ˈˈ 당신이 관심 둘 만한 물품을 가져왔어요\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
execute as @s[tag=completeTrackedQuest] run scoreboard players set illegalItems sharedQuests -1
