...
execute as @s[scores={delayConvTellraw=2,playerID=1}] run scoreboard players operation @s convState = harryPotter p1convOverride
execute as @s[scores={delayConvTellraw=2,playerID=2}] run scoreboard players operation @s convState = harryPotter p2convOverride
execute as @s[scores={delayConvTellraw=2,playerID=3}] run scoreboard players operation @s convState = harryPotter p3convOverride
execute as @s[scores={delayConvTellraw=2,playerID=4}] run scoreboard players operation @s convState = harryPotter p4convOverride
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"[{\"text\":\" 안녕! 나는 \"},{\"text\":\"해리 포터야\",\"color\":\"gold\",\"bold\":\"true\"},{\"text\":\".\\n\\n\\n\\n\\n\",\"color\":\"white\",\"bold\":\"false\"}]"}
...
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=1}] run scoreboard players set harryPotter p1convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=2}] run scoreboard players set harryPotter p2convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=3}] run scoreboard players set harryPotter p3convOverride 1
execute as @s[tag=trg] as @e[type=armor_stand,limit=1,scores={con=1}] as @s[tag=convSelected] as @p[tag=activePlayer,scores={tmp=0,playerID=4}] run scoreboard players set harryPotter p4convOverride 1
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 뭐 필요한 거라도 있어? \\n\"}"}
# execute if entity @s[tag=use] run tag @s add longHeader
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"≤ 네가 해리 포터야?\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=2}] run data merge entity @s {CustomName:"{\"text\":\"≤ 호그와트는 어때?\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=3}] run data merge entity @s {CustomName:"{\"text\":\"≤ 퀴디치 할 줄 알아?\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 그래. 내 이름이 마법 세계에서 무슨 의미인지 알아. 처음에는 나도 놀랐어. 아무리 상관 안 하고 내 삶을 살려고 노력해도, 매년 볼드모트가 약속한 것처럼 나타나서 잊어버릴 수가 없어. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 그럼. 호그와트 팀의 수색꾼이야. 일 학년 때부터 쭉. 그때 말포이가 네빌의 리멤브럴을 훔치고 빗자루 타고 도망가서, 내가 그 뒤를 쫓았거든. 빗자루 처음 타는 것이기도 하고, 큰일 났다고 생각했는데 그 반대였지. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
