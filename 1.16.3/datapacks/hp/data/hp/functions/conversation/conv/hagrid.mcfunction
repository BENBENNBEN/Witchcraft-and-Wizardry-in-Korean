...
################################
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"±ˈˈ 네, 학교 물품을 사야 해서요.\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"[{\"text\":\" 아, 호그와트 신입생이니? 그렇담 내 소개를 해야겠구나. 나는\\n\"},{\"text\":\" 루비우스 해그리드야.\",\"color\":\"gold\",\"bold\":\"true\"},{\"text\":\" 호그와트의 열쇠 지킴이이자 숲지기란다.\\n\\n\\n\\n\"}]"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"[{\"text\":\" 다이애건 앨리에 들어가면 \"},{\"text\":\"그린고트 은행부터 가.\",\"color\":\"gold\",\"bold\":\"true\"},{\"text\":\" 네가 가진 머글 돈을 마법 세계의 화폐로 환전해야 할 거야. 이 마법 세계에서 그린고트처럼 안전한 곳은 없으니까 안심하고. 물론 호그와트를 제외하고서 말이야. 가서 그립훅을 찾고서 널 도와달라고 하렴.\"}]"}
execute if entity @s[tag=use] run tag @s add longHeader
