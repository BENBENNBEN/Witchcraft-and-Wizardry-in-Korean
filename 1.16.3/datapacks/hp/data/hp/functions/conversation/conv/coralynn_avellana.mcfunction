...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 다이애건 앨리에는 마법약 재료를 파는 곳이 많아. 직접 재료를 구하러 가기 싫을 때 좋지. 우리 가게에 찾는 재료가 없으면 내 친구 윌튼 햄록이 하는 가게로 가보렴. 조금만 가면 있는 슬러그 앤 지글러 마법약 상점이야. \\n \"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" 아니면 멀페퍼 씨의 상점도 있고. 윌튼이랑 사이 안 좋아서 추천하기 그렇지만. \\n\\n\\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
